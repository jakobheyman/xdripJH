    ...
    public static boolean is_new(SensorRecord sensorRecord, long addativeOffset) {
        ...
        if (sensor != null) {
            BgReading bgReading = new Select()
                    .from(BgReading.class)
                    .where("Sensor = ? ", sensor.getId())
                    .where("timestamp <= ?", (timestamp + (15 * 1000))) // 15 seconds padding (should never be that far off, but why not)
                    .orderBy("timestamp desc")
                    .executeSingle();
            if (bgReading != null && Math.abs(bgReading.timestamp - timestamp) < (15 * 1000)) { //cool, so was it actually within 0.25 minutes of that bg reading?
